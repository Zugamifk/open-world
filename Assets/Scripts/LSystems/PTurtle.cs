...
namespace LSystems
{
    public class PTurtle : Turtle3D
    {
        ...
        public struct AugmentedVertex
        {
            ...
            public AugmentedVertex(Vector3 position, Quaternion rotation, PLSystem.Word word) : this(position, word)
            {
                    ...
                    default:
                        {
                            if (current.name[0].IsUpper())
                            {
                                points.Add(new AugmentedVertex(turtleTransform.localPosition, turtleTransform.localRotation, derivation[i]));
                                if (lastpt >= 0)
                                {
                                    connections.Add(lastpt);
                                    connections.Add(currentpt);
